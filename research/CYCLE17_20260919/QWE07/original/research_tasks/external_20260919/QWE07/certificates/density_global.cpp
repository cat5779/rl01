...
using namespace std;
int main(int argc,char**argv){fesetround(FE_UPWARD);I c=rat(19,20),rho_lo=rat(1,3),rho_hi=argc>2?rat(stoll(argv[1]),stoll(argv[2])):rat(1009,3000);uint64_t all=0;for(int cell=0;cell<60;cell++){
 I al=rat(420+cell,20000),ah=rat(421+cell,20000);
 I dmax=(al+c)/al,dmin=(I(1)-ah-c)/(I(1)-ah);
 I LL=al+c*rho_lo/(rho_lo+dmax*(I(1)-rho_lo)),UU=ah+c*rho_hi/(rho_hi+dmin*(I(1)-rho_hi));
