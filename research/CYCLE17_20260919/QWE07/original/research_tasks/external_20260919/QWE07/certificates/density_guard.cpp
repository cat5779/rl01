...
Mat2 guardmat(int rad,unsigned word,I a,bool upper,long long rn,long long rd){vector<int>co={0,1};for(int i=-rad;i<0;i++)co.push_back(i);for(int i=2;i<=rad+1;i++)co.push_back(i);int n=co.size();auto q=sine_Q(co,rn,rd);I c=rat(19,20),dmax=(a+c)/a,dmin=(I(1)-a-c)/(I(1)-a),d=upper?dmin:dmax;
...
constexpr long long ADEN=1024000,RDEN=768000;
int cap_lo[256],cap_hi[256];uint64_t totalnodes=0,leaf_count=0,split_count=0;int failed=0;
I wordcap(int z,I a){return ((rat(3,100)-a)*I(cap_lo[z])+(a-rat(1,50))*I(cap_hi[z]))/I(100);}
bool cell_proof(int word,long long al,long long ah,long long rl,long long rh,int da=0,int dr=0){
 I a0=rat(al,ADEN),a1=rat(ah,ADEN);auto L=guardmat(4,word,a0,false,rl,RDEN),U=guardmat(4,word,a1,true,rh,RDEN);
 R cap=min(wordcap(word,a0).l,wordcap(word,a1).l);
 auto rep=certify(L,U,I(a0.l,a1.h),cap,2000000);
 #pragma omp atomic
 totalnodes+=rep.nodes;
 if(rep.pass){
  #pragma omp critical
  {leaf_count++;cout<<setprecision(21)<<"LEAF "<<word<<" "<<al<<" "<<ah<<" "<<ADEN<<" "<<rl<<" "<<rh<<" "<<RDEN<<" nodes "<<rep.nodes<<" cap "<<cap<<"\n";cout.flush();}
  return true;
 }
 #pragma omp atomic
 split_count++;
 if(da<4){long long mid=(al+ah)/2;bool l=cell_proof(word,al,mid,rl,rh,da+1,dr);bool r=cell_proof(word,mid,ah,rl,rh,da+1,dr);return l&&r;}
 if(dr<4){long long mid=(rl+rh)/2;bool l=cell_proof(word,al,ah,rl,mid,da,dr+1);bool r=cell_proof(word,al,ah,mid,rh,da,dr+1);return l&&r;}
 #pragma omp critical
 {cerr<<"FAILED_BOX "<<word<<" "<<al<<" "<<ah<<" "<<ADEN<<" "<<rl<<" "<<rh<<" "<<RDEN<<" bound "<<rep.worst<<" cap "<<cap<<endl;}
 return false;
}
int main(int argc,char**argv){fesetround(FE_UPWARD);int first=argc>1?stoi(argv[1]):0,last=argc>2?stoi(argv[2]):1536,threads=argc>3?stoi(argv[3]):4;
 if(first<0||last>1536||first>=last)throw runtime_error("job bounds");
 ifstream in("guard_caps_integer.txt");for(int i=0;i<256;i++)if(!(in>>cap_lo[i]>>cap_hi[i]))throw runtime_error("guard input");
 auto t0=chrono::steady_clock::now();omp_set_num_threads(threads);int jobs=0;
 #pragma omp parallel for schedule(dynamic)
 for(int job=first;job<last;job++){fesetround(FE_UPWARD);int cell=job/256,word=job%256;bool ok=false;
  try{ok=cell_proof(word,21504+512*cell,21504+512*(cell+1),256000,258304);}catch(exception&e){
   #pragma omp critical
   {cerr<<"ERROR job "<<job<<" "<<e.what()<<endl;}
  }
  #pragma omp critical
  {jobs++;if(!ok)failed++;cout<<"JOB "<<job<<" "<<(ok?"PROVED":"FAILED")<<"\n";cout.flush();}
 }
 cerr<<"SUMMARY jobs "<<jobs<<" failed "<<failed<<" leaves "<<leaf_count<<" splits "<<split_count<<" nodes "<<totalnodes<<" seconds "<<chrono::duration<double>(chrono::steady_clock::now()-t0).count()<<endl;
 return failed?2:0;
}
