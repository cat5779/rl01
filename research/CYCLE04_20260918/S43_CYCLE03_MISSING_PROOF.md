...
COMPLETE VISIBLE SOURCE-RECOVERY RESPONSE acquired (17,130 characters). Restates the omitted count-Stein proof and final assembly. Author-source delivery, not independent certification. Earlier scoped review PR15 must be extended to this proof; attachment body itself not acquired.
...
\tag{2}
\]
The last equality uses \(F_{n-l}=F_l\) and the prescribed zero entropies at \(0,1,n-1,n\). No layer law is differentiated in (2). citeturn452233view0
...
For example,
\[
H(Q_t)=-\mathbb E_t\log Q-t\mathbb E_tX+
\log\mathbb E_Qe^{tX},
\]
whose second derivative at zero is
...
\[
\mathbb E\mathcal L\log Q=-\sum_i\mathbb E[x_ih_i].
\]
Consequently (4)–(5) imply
\[
\boxed{
\mathcal D_n''(0)
...
\tag{6}
\]
This is the recovered exact susceptibility identity. fileciteturn1file0L161-L189
...
\tag{12}
\]
This is the finite signed certificate producing \(L_c\) and, in the limit, \(A_c\). fileciteturn1file0L199-L239
...
For the count law
\[
\pi_l=[t^l]\{b+(1-2b)t+bt^2\}^k,
\]
exponential tilting to mean \(l\) in this band keeps all Bernoulli parameters in a fixed compact subset of \((0,1)\). Its mass at the tilted mean is between \(c_1/\sqrt n\) and \(c_2/\sqrt n\), uniformly, by Fourier inversion. The rate function has second derivative \(1/\operatorname{Var}_tN=O(n^{-1})\). Thus
\[
|\log\pi_{k-r}-\log\pi_k|\le C(1+r^2/n).
\]
Adjacent binomial ratios likewise give
\[
\left|\log\binom n{k-r}-\log\binom nk\right|\le Cr^2/n.
\]
Since
\[
...
\tag{15}
\]
Symmetry gives the upper band; outside the bands, \(0\le\mathscr F_l\le n\log2\) gives the same envelope. No entropy approximation has been differentiated. fileciteturn1file0L18-L57
...
\tag{16}
\]
Explicitly, the first \(a\)-score is \(X/b\). If \(T\) is the balanced \(c\)-score, the derivative of the first \(a\)-score with respect to \(a\) at the midpoint is
\[
-\frac nb+\frac{2c}{b}T,
\]
proving (16).
...
\tag{21}
\]
This is the recovered actual-count/field conversion. fileciteturn1file0L107-L144
...
\theta_l=
\frac{(z-1)^2[t^{l-2}](1+t)^{k-2}(1+zt)^{k-2}}
{[l(l-1)/(k(k-1))]Z_l}.
\]
...
\]
These are the exact-clock radial KL and third-difference evidence-flatness inputs. The radial proof’s external input was strong-Rayleigh preservation under finite symmetric exclusion; no S45 conclusion is assumed. Statement (25) is not an assertion that \(U_l\) converges or has a signed derivative. fileciteturn2file0L7-L29 fileciteturn2file1L34-L54

For completeness, the actual-weight input needed to pay the complement is
\[
\frac1n\sum_l(\kappa_l)_+\to M_b,\quad
\frac1n\sum_l|\kappa_l|\to2M_b,\quad
...
\[
\sum_l(\kappa_l)_+=2\max_m(B_m-B_{m-1}).
\]
Fourier inversion gives
\[
\frac{B_{k-r-1}-B_{k-r-2}}n
\longrightarrow \frac{x}{b}\varphi_b(x)
\quad\text{for }r/\sqrt n\to x,
\]
uniformly over the lattice, where \(\varphi_b\) is the \(N(0,b)\) density. Its maximum is \(e^{-1/2}/(\sqrt{2\pi}b)\), proving (26).
...
\left[1+\left(\frac{l-k}{\sqrt n}\right)^4\right]
\]
uniformly, and implies (27). Thus the complement payment is for the actual coefficients, not substituted Gaussian weights. fileciteturn2file2L64-L83
...
\tag{29}
\]
This reproduces the signed output-KL budget without assigning a sign to an adjacent-layer KL difference. fileciteturn1file0L252-L278
...
\[
0<D_c<\frac{1-\log2}{2}<\frac16,\qquad
...
\tag{31}
\]
This is the recovered analytic positivity argument; it does not depend on decimal evaluations. fileciteturn1file0L286-L302
