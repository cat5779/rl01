...
inline R log2upper(){R t=ud(1,3), t2=um(t,t),p=t,s=0;for(int k=0;k<30;k++){s=ua(s,ud(p,2*k+1));p=um(p,t2);}R tail=ud(p,dm(61,da(1,-t2)));return um(2,ua(s,tail));}
inline R logupper(R x){if(x<1)throw std::runtime_error("log below one");if(x==1)return 0;int e;frexpl(x,&e);int k=e-1;R z=ldexpl(x,-k);R t=ud(ua(z,-1),da(z,1));R t2=um(t,t),p=t,s=0;for(int j=0;j<24;j++){s=ua(s,ud(p,2*j+1));p=um(p,t2);}R tail=ud(p,dm(49,da(1,-t2)));R local=um(2,ua(s,tail));return ua(local,um(k,log2upper()));}
inline R lfunup(R x){if(x<.0001L)return ua(1,um(x,.5L));R z=ua(1,x);return ud(um(z,logupper(z)),x);}
inline std::vector<I> invmat(std::vector<I> a,int n){std::vector<I>b(n*n);for(int i=0;i<n;i++)b[i*n+i]=I(1);for(int k=0;k<n;k++){int p=k;R best=0;for(int j=k;j<n;j++){R mid=(a[j*n+k].l+a[j*n+k].h)/2;if(fabsl(mid)>best){best=fabsl(mid);p=j;}}if(p!=k)for(int j=0;j<n;j++){std::swap(a[k*n+j],a[p*n+j]);std::swap(b[k*n+j],b[p*n+j]);}I pivot=a[k*n+k];if(pivot.l<=0&&pivot.h>=0)throw std::runtime_error("ambiguous inverse");for(int j=0;j<n;j++){a[k*n+j]=a[k*n+j]/pivot;b[k*n+j]=b[k*n+j]/pivot;}a[k*n+k]=I(1);for(int i=0;i<n;i++)if(i!=k){I f=a[i*n+k];for(int j=0;j<n;j++){a[i*n+j]=a[i*n+j]-f*a[k*n+j];b[i*n+j]=b[i*n+j]-f*b[k*n+j];}a[i*n+k]=I(0);}}return b;}
