// S70 guard-only fast log enclosure: 12 atanh terms, matching tail; cached log2.
#pragma once
#include <bits/stdc++.h>
#include <cfenv>
...
inline R log2upper(){static thread_local R cached=[](){// same rigorous sum, cached per thread
 R t=ud(1,3), t2=um(t,t),p=t,s=0;
 for(int k=0;k<30;k++){s=ua(s,ud(p,2*k+1));p=um(p,t2);}
 R tail=ud(p,dm(61,da(1,-t2)));return um(2,ua(s,tail));}();return cached;
}
inline R logupper(R x){if(x<1)throw std::runtime_error("log below one");if(x==1)return 0;int e;frexpl(x,&e);int k=e-1;R z=ldexpl(x,-k); // exact power-of-two scaling, z in [1,2)
 ...
 for(int j=0;j<12;j++){s=ua(s,ud(p,2*j+1));p=um(p,t2);}
 R tail=ud(p,dm(25,da(1,-t2)));R local=um(2,ua(s,tail));
