#!/usr/bin/env python3
"""
S73 reproducibility checks.

Exact component:
  c=19/20, a=(1-c)/2=1/40, four-site cyclic half-density projection
  (m=2 parity sites).  The entropy-gain curvature is certified positive by
  an integer inequality.

Floating-point diagnostics:
  constructs the 2m-site cyclic Fourier projection, its parity unitary U,
  the exterior-power channel W_U, and checks the compensated curvature
  identity by direct enumeration.

Python 3.11+; NumPy required only for the floating-point section.
"""
...
def exact_m2_certificate():
    c = Fraction(19, 20)
    r = (1 - c * c) / 4       # q_00=q_11 at midpoint
    s = (1 + c * c) / 4       # q_01=q_10 at midpoint

    # For the 2x2 cyclic parity unitary, the Hamming-one exterior channel is
    # the uniform 2x2 bistochastic matrix.  Direct exact differentiation gives
    # G'' = 8 s log(((r^2+s^2)^2)/(4 r^2 s^2)).
    numerator = (r * r + s * s) ** 2
...
def enumerated_curvature(m, a, c):
    ...
    # Exact compensated identity:
    # G'' = E[A log(P/Q)] + E Var(score | output).
    term_acc = 0.0
    term_fisher = 0.0
    for iy in range(n):
        for it in range(n):
