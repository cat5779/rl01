...
不能只检测两律不同：尝试构造二阶导数稳定性工具、模式缺陷传递算子或相对熵比较泛函。若否定一个桥接候选，交付明确对象及其失效机制，并指出可据此检验的修正方向。

- P1：同时输入真实律与指定修正律的必要 jets，满足归一与移动权重规则，不用单一 degree-two 匹配冒充全律相等。
- P2：两族及所需 jets 相同时缺陷消失；给出未匹配模式、移动参考与非线性误差的明确表达。
- P3：证明一个局部或单侧的二阶熵比较引理，列明所需正性/可积性和常数的 n 依赖；o(n) 是升级目标而非预设条件。

域外种子：PDE 相对熵/调制能量方法、半群扰动的 Duhamel 展开、线性响应与模型降阶误差分析。必须核对参数二阶微分条件；不能由静态距离小直接推出 Hessian 接近，也不能把所求桥接界藏在迁移前提中。
...
\]
\[
\widehat q_l(S)=u_l(S)[e^{\widehat\tau_l L_l}r_l^{max}](S).
\]
...
需要数值诊断时，从 n=4、6 中选一个有非零待测模式的最小实例；若模式因对称性消失，先解释结构原因。更大规模及系统枚举写入计算交接，不由发现者自动扩算。
...
可选背景附件：`sources/S13_PROOF.md`、`S13_TERM_BALANCE.md`、`S13_AUDIT.md`。
