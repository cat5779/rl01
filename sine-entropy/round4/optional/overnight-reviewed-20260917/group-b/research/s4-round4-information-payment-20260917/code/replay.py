...
def main():
    ...
    for script,filename in [('exact_check.py','exact_certificate.json'),
        ...
        cmd=[sys.executable,str(CODE/script),'--out-dir',str(out)]
        ...
        actual=load(out/filename);frozen=load(RECEIPTS/filename)
        ...
        text=(RECEIPTS/filename).read_text(encoding='utf-8')
    ...
    guard=subprocess.run([sys.executable,str(CODE/'exact_check.py'),'--quick',
                          '--out-dir',str(RECEIPTS/'forbidden_output')],
    ...
    require(not (RECEIPTS/'forbidden_output').exists(),'Guard created an evidence directory')
    if args.with_floating:
        env=dict(os.environ);env['OPENBLAS_NUM_THREADS']='1'
        cmd=[sys.executable,str(CODE/'floating_checks.py'),'--out-dir',str(out/'floating')]
