...
### 7. 22 点最近邻框架不能只靠更换测试函数继续推进

当前有限窗证明框架里还有一个严格的方法反例。固定

\[
\rho=\frac13,\qquad c=\frac{19}{20},\qquad a=\frac{13}{500}=0.026,
\]

继续使用同一个 22 点窗口、带 guard 的最近邻 pair，以及仿射全局帽

\[
C^\dagger(a)=\frac{6501}{5000}+\frac{2067}{400}a+\frac1{1000}.
\]

外向区间证书覆盖整个 `[0.0255,0.026]`：全局 pair 上包络成立，256 个 guard word 全部通过，而且每个分配矩阵 `B_z` 都半正定。因此在 `a=0.026` 失效的不是 cap、guard 或 PSD 合法性，而是这个固定框架能够提供的局部支付量。

记 `Z_A` 为 22 点实际律在目标两坐标上的 score。对完整 22 点 word 的任意二向量测试函数 `f`，都有

\[
2Z_A^{\mathsf T}B_zf-f^{\mathsf T}B_zf
=Z_A^{\mathsf T}B_zZ_A
 -(Z_A-f)^{\mathsf T}B_z(Z_A-f)
\le Z_A^{\mathsf T}B_zZ_A.
\]

所以在分配不变时，取 `f=Z_A` 已经达到所有测试函数中的最大支付。对全部 `2^{22}=4,194,304` 个配置作完整外向枚举，得到
...
这严格否定了“只换一个更好的二向量测试函数，就能让同一 22 点最近邻分配穿过 `a=0.026`”的设想。若要继续上推，必须同时引入新的资源，例如更尖锐的全局 cap、更大的条件 score 窗口，或包含额外边类的不同 PSD 分配。它不是熵率非凹反例，也不排除这个固定框架之外的证明。

### 8. 真实六点 sine 后验否定零支付条件 Jensen

这个反例发生在目标模型内部，不是任意构造的收缩核。取六个真实站点
...
在站点 `A={0,1,2,3,4}` 上观察到正概率 word

\[
y=(0,0,0,1,1),
\]

保留核心 `I={2,3}`，再揭示第 5 个站点。令

\[
t=\frac{19}{20\pi},\qquad
A_y=
\begin{pmatrix}
-\frac12&t&0&-\frac t3&0\\
t&-\frac12&t&0&-\frac t3\\
0&t&-\frac12&t&0\\
-\frac t3&0&t&\frac12&t\\
0&-\frac t3&0&t&\frac12
\end{pmatrix},qquad
b=\begin{pmatrix}\frac t5&0&-\frac t3&0&t\end{pmatrix}^{T}.
\]

写

\[
G=A_y^{-1},\qquad M=G_{II},\qquad v=(Gb)_I,
\qquad q=\frac12-b^TGb.
\]

那么揭示最后一个站点后的两个真实条件 score 块恰为

\[
M_1=M+\frac{vv^T}{q},\qquad
M_0=M-\frac{vv^T}{1-q},
\]

其实际条件概率分别是 `q` 与 `1-q`，并且

\[
qM_1+(1-q)M_0=M.
\]

因此这不是把不可能出现的两个矩阵硬凑成平均，而是同一个 sine-Toeplitz 输出过程中的真实一步后验分裂。

为明确被否定的命题，对 Hermitian 矩阵 `X` 定义

\[
f_{ij}(X)=|X_{ij}|^2+
\bigl(X_{ii}X_{jj}-|X_{ij}|^2\bigr)
\log\!\left(1-\frac{|X_{ij}|^2}{X_{ii}X_{jj}}\right),
\]

\[
\Phi(X)=\sum_iX_{ii}^2+\sum_{i\ne j}f_{ij}(X),
\qquad
\operatorname{Chi}(X)=\sum_{i\in E,\,j\in O}f_{ij}(X).
\]

这里 `Phi` 的 pair 和按有序对计数，`Chi` 对每个偶—奇 pair 只计一次。若零支付条件 Jensen 成立，揭示第 5 个站点就应使下面两个 Jensen 差非负：
...
\[
\Delta_{\operatorname{Chi}}=q\operatorname{Chi}(M_1)
+(1-q)\operatorname{Chi}(M_0)-\operatorname{Chi}(M).
\]

但 256 位、全程外向舍入的有理区间证书给出
...
| `Q=||v||^4/[q(1-q)]` | `[0.170309619791317635, 0.170309619791317636]` |

特别地，`Delta_Phi<0` 且 `Delta_Chi<0`。所以“揭示更多站点时，完整势 `Phi` 或偶奇势 `Chi` 无需支付便按条件 Jensen 单调改善”这一证明路线，在真实目标模型中已经被严格否定。即使允许二次支付

\[
\Delta_\Phi\ge-\lambda\mathcal Q,
\]

这个 word 也强迫

\[
\lambda\ge 0.625832574752639508>\frac58.
\]

反例的边界同样重要：它只否定**逐次揭示时的零支付 Jensen**。它不否定带 Hessian 支付的观察比较，不决定对全部 word 取实际概率平均后的符号，也不决定 `V_{m,L}` 或真实 sine-Toeplitz 熵率的凹性。
